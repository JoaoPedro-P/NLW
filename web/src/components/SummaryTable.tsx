...
import { GenerateRangeDays } from "../util/generate-range-days"
...
const arrayDays = [
    ...
    const amoutOfDaysToFill = minimunSummaryDatesSize - summaryDates.length
    ...
    return (
        <div className="w-full flex">
            <div className="grid grid-rows-7 grid-flow-row gap-3">
                {arrayDays.map((arrayDays, i) => {
                    return (
                        <div key={`${arrayDays}-${i}`}className="text-zinc-400 text-xl h-10 w-10 flex items-center justify-center">
                            {arrayDays}
                        </div>
                    )
